    ...
    internal class Program
    {
        ...
        private static async Task<int> Main(string[] args)
        {
            var parser = new CommandLineBuilder(new Command("dotnet-format", handler: CommandHandler.Create(typeof(Program).GetMethod(nameof(Run)))))
                ...
                .AddOption(new Option(new[] { "-fs", "--fix-style" }, Resources.Run_code_style_analyzer_and_apply_fixes, new Argument<bool>()))
        ...
        public static async Task<int> Run(string folder, string workspace, string verbosity, bool fixStyle, bool dryRun, bool check, string files, IConsole console = null)
        {
            // Setup logging.
